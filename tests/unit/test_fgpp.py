"""Unit tests for the get_fgpp MCP tool."""
...
from legacy_mcp.tools import fgpp as fgpp_module
...
def tools(workspace: Workspace) -> _MockMCP:
    mcp = _MockMCP()
    fgpp_module.register(mcp, workspace)
    return mcp
...
class TestGetFgpp:
    ...
    def test_items_have_expected_keys(self, tools: _MockMCP) -> None:
        ...
        for item in result["items"]:
            assert "Name" in item
            assert "Precedence" in item
            assert "MinPasswordLength" in item
