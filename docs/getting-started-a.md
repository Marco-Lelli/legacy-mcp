...
> generated by pip inside `.venv\Scripts\` (uvicorn.exe, python.exe and
> package entry points). This is not a LegacyMCP issue — it affects any
> Python virtual environment created on managed Windows.
>
> If you encounter errors or missing executables after `pip install`:
>
> 1. Ask your IT administrator to add an exclusion for the `.venv\Scripts\`
>    folder in the endpoint protection policy.
> 2. If you have local administrator rights, you can add the exclusion yourself:
>    ```powershell
>    Add-MpPreference -ExclusionPath "$PWD\.venv\Scripts"
>    ```
>    Note: this requires elevation and may be overridden by Group Policy.
> 3. If neither option is available, create the virtual environment on a
>    machine outside the corporate endpoint policy, copy the entire `.venv`
>    folder to the target machine, and run `legacy-mcp --help` to verify.
