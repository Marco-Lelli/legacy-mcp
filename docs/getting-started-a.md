...
open a PowerShell window **as Administrator** in the `installer\` subfolder.
...
The installer creates the virtual environment in
`%ProgramFiles%\LegacyMCP\.venv`, installs the package, and writes
the configuration paths to the Windows registry. Writing to `Program Files`
...
> generated by pip inside `%ProgramFiles%\LegacyMCP\.venv\Scripts\`
...
>    `%ProgramFiles%\LegacyMCP\.venv\Scripts\` folder in the endpoint
>    protection policy.
> 2. If you have local administrator rights, you can add the exclusion yourself:
>    ```powershell
>    Add-MpPreference -ExclusionPath "$env:ProgramFiles\LegacyMCP\.venv\Scripts"
>    ```
...
>    and copy the resulting `%ProgramFiles%\LegacyMCP\.venv` to the target machine.
...
The installer prints this block at the end of Phase 6. Copy it to your
...
```json
{
  "mcpServers": {
    "legacymcp": {
      "command": "C:\\Program Files\\LegacyMCP\\.venv\\Scripts\\python.exe",
...
The configuration path is read automatically from the Windows registry
(written by the installer). Use double backslashes in JSON.
...
The installer creates `%ProgramData%\LegacyMCP\config\config.yaml` from
...
5. Update `%ProgramData%\LegacyMCP\config\config.yaml` to point to the real JSON:
...
- Verify the Python path in `claude_desktop_config.json` is absolute
  and points to `%ProgramFiles%\LegacyMCP\.venv\Scripts\python.exe`.
- Check that `%ProgramData%\LegacyMCP\config\config.yaml` exists and
...
- Run the server manually to see the error:
  ```powershell
  & "$env:ProgramFiles\LegacyMCP\.venv\Scripts\python.exe" -m legacy_mcp.server
  ```
